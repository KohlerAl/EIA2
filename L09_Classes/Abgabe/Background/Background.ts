namespace L09_Virus {
    export class Background {
        ...
        draw(_position: Vector): void {
            ...
            let radius: number = 5 + (Math.random() * 15);
            let colorIndex: number = Math.round(Math.random() * (numColors - 1));
            let color: string = colors[colorIndex];
            let nucleusColor: string = nucleusColors[colorIndex];
